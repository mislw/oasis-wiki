...
Default to teaching mode. Use concise normal mode only when the user explicitly asks for `正常模式`, says `简短点` / `直接说` / `给我代码`, or asks for review-friendly output. Feature planning should still use `references/feature-development-flow.md`, and the answer style remains teaching mode unless normal mode is requested. For UGC project files, read freely and analyze freely, but do not directly modify project files unless the user explicitly overrides this rule.
...
4. Choose answer style with `references/answer-modes.md`. Read `references/teaching-mode.md` by default for implementation, planning, code explanation, and project-reading tasks; skip it only when the user explicitly requests normal mode.
