---
name: oasis-wiki
description: Use when answering questions or writing code about Oasis/绿洲启元/和平精英 UGC/RedCliff Lua development, including API lookup, gameplay systems, UI systems, editor workflows, examples, templates, debugging, performance, release notes, and terminology. Search the bundled local wiki before giving technical guidance or code for these domains.
---

# Oasis Wiki

Use this skill for Oasis/绿洲启元 and RedCliff development questions. The bundled wiki is the source of truth for Lua APIs, editor workflows, gameplay systems, UI, templates, troubleshooting, and examples.
...
3. For implementation answers, cite the relevant local file paths and line numbers when possible.
4. When writing Lua, match the wiki examples and API naming exactly. If a detail is not found, say that the local wiki did not confirm it.
5. For broad questions, synthesize from 2-4 relevant files rather than one giant context load.
